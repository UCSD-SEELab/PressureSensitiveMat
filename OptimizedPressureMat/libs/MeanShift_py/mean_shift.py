import numpy as np
import sys
sys.path.append('./libs/MeanShift_py')
...
class MeanShift(object):
    ...
    def cluster(self, points, kernel_bandwidth, pressureWeights, iteration_callback=None):
        ...
        while max_min_dist > MIN_DISTANCE and np.linalg.norm(old_max_min_dist - max_min_dist) > 0.0001 and iteration_number <50:
            print('old_max_min_dis is ' + str(old_max_min_dist))
            print('max_min_dis is ' + str(max_min_dist))
            print('diff is ' + str(np.linalg.norm(old_max_min_dist - max_min_dist)))
            # print max_min_dist
            ...
            for i in range(0, len(shift_points)):
                ...
                p_new_start = p_new
                p_new = self._shift_point(p_new, points, kernel_bandwidth, pressureWeights)
    ...
    def _shift_point(self, point, points, kernel_bandwidth, pressure_weights):
        # from http://en.wikipedia.org/wiki/Mean-shift
        points = np.array(points)

        # numerator 1*pressure_weights*
        #print('pressure weight' + str(pressure_weights))
