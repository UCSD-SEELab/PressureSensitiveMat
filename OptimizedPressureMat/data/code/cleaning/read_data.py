...
class RawDataDigester(object):
    def __init__(self, path):
        ...
        for line in lines:
            ...
            try:
                ...
                print
                "PARSE ERROR IN: {}".format(line)
                raise e
            counter += 1

        if len(parse_errors) > 0:
            print
            "WARNING: {} MESSAGES COULD NOT BE PARSED".format(len(parse_errors))
            for msg in parse_errors:
                print
                msg
        print
        "PARSED: {} MESSAGES".format(counter)
    ...
    def get_plugs_data(self):
        return self.data['plug1'], self.data['plug2'], self.data['plug3'], self.data["tv_plug"], self.data[
            "teapot_plug"]
